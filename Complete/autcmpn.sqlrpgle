...
dcl-proc check_obja;
    ...
    dcl-s data char(100);
    //
    stmt = 'select oausr, oaobja, oagrpn from '+ %trim(target_libnm) + '.' + %trim(target_filnm) + ' ' +
                'where oalib = ? ' +
                'and oaname = ? ' +
                'and oatype = ?';
    ...
    exec sql fetch next from autcur into :fil_usr, :fil_objobja, :fil_grpn;
    dow sqlcod = 0;
        if sqlcod = 0;
            if fil_usr = '*GROUP';
